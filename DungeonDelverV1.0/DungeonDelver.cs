...
namespace DungeonDelver 
{
    ...
    public class GameManager 
    {
        ...
        private Player OutputIntroduction()
        {
            ...
            Armor starterArmor = new Armor();
            player.EquipArmor(starterArmor);
            ...
            Console.WriteLine($"You stumble into a skeleton that still has some armor on it. You found a {starterArmor.ArmorName}");
        ...
        private void OutputCombatLog(Enemy enemy)
        {
            ...
            void generateLootChances(){
                Random random = new Random();
                int lootChance = random.Next(101);

                if (lootChance <= 50) {
                    Armor droppedLoot = enemy.DropArmorLoot();
                    Console.WriteLine($"You search it's corpse and find a {droppedLoot.ArmorName}");
                    player.EquipArmor(droppedLoot);
                    Console.WriteLine("Your defense rating is now: " + player._defense);
                    for (int i = 0; i < player.equipedArmor.Count; i++) {
                    Console.WriteLine("You have " + player.equipedArmor[i].ArmorName + " equipped");
                    }
                }
                else {
                    Consumable droppedLoot = enemy.DropConsumableLoot();
    ...
    public class Player 
    {
        ...
        public bool Attack(Enemy enemy)
        {
            ...
            bool hit = hitChance <= 90; // 10% miss chance
        ...
        {
            int finalDMG = damage - Convert.ToInt32(Math.Ceiling(_defense / 5.0) * 0.5);
        ...
        public void EquipArmor(Armor armor){
            //bool inInventory = equipedArmor[0].ArmorName.Contains(armor.ArmorType);
            //bool inItem = false;
            
            bool alreadyEquiped = false;
            if (equipedArmor.Count <= 4){
                for (int i = 0; i < equipedArmor.Count; i++){
                    if (armor.ArmorType == equipedArmor[i].ArmorType){
                            alreadyEquiped = true;
                    }
                }
                if (!alreadyEquiped) {
                    equipedArmor.Add(armor);
    ...
    public class Enemy
    {
        ...
        Consumable consumableloot = new Consumable();
        Armor armorLoot = new Armor();
        ...
        public void GenerateBoss()
        {
            this.enemyType = "BOSS";
            this.Health = 300;
            this.attackPower = random.Next(30, 40);
        ...
        public Consumable DropConsumableLoot()
        {
            return consumableloot;
        }
        public Armor DropArmorLoot(){
    ...
    public class Weapon
    {
        ...
        string[] weaponNames = new string[] 
        {
            ...
            "Greatsword", "Warhammer", "Sickle", "Falchion", "Rapier", "Profaned Short Sword",
            "Club", "Flail", "Halberd", "Hatchet", "Javelin", "Katana", "Trident"
        };
    ...
    public class Armor{
        ...
        public string[] StringArmorType = new string[]{
            "Helment", "Chestplate", "Greaves", "Guantlets"
        };

        string[] ArmorMaterial = new string[]{
            "Iron", "Steel", "Obsidian", "Titanium", "Cosmic" 
        };

        string[] rarityPrefix = new string[]{
            "COMMON", "UNCOMMON", "RARE", "LEGENDARY", "DIVINE"
        };

        public Armor(){
            string randArmor = StringArmorType[random.Next(StringArmorType.Length)];
            ...
            calculateArmorRarity();
            if(calculateBroken()){
                Rarity += " Broken";
            }
            calculateArmorMaterial();

            
            ArmorName = Rarity + " " + randArmor;
            
            

        }

        public string calculateArmorRarity(){
            int randRarity = random.Next(101);

            if (randRarity <= 50 && randRarity >= 0){
                return returnResult(0, 0);
            }
            else if (randRarity <= 70 && randRarity >= 51){
                return returnResult(1, 3);
            }
            else if (randRarity <= 85 && randRarity >= 71){
                return returnResult(2, 5);
            }
            else if (randRarity <= 95 && randRarity >= 86){
                return returnResult(3, 8);
            }
            else if (randRarity <= 100 && randRarity >= 96){
                return returnResult(4, 15);
            }

            string returnResult(int index, int defense) {
                Rarity = rarityPrefix[index];
                Defense += defense;
                return rarityPrefix[index];
            }

            return rarityPrefix[0];
        }

        public string calculateArmorMaterial(){
            int randType = random.Next(101);
            if (randType <= 50 && randType >= 0){
                return returnResult(0, 0);
            }
            else if (randType <= 70 && randType >= 51){
                return returnResult(1, 1);
            }
            else if (randType <= 85 && randType >= 71){
                return returnResult(2, 2);
            }
            else if (randType <= 95 && randType >= 86){
                return returnResult(3, 3);
            }
            else if (randType <= 100 && randType >= 96){
                return returnResult(4, 5);
            }

            string returnResult(int index, int defense) {
                Rarity += " " + ArmorMaterial[index];
                Defense += defense;
                return ArmorMaterial[index];
            }

            return "";
        }
    
        public bool calculateBroken(){
            int randBroken = random.Next(101);

            if (randBroken <= 15){
                Defense = Convert.ToInt32(Math.Floor(Defense / 2.0));
                return true;
            }
            else {
                return false;
